...
# this function is used as divide input data and labels in mini batch(batchsize)
# and also used shuffle to give some randomness to CNN
def iterate_minibatches(inputs, targets, batchsize, shuffle=False):
    assert len(inputs) == len(targets)
    if shuffle:     # shuffle is used in train the data
...
# {conv(with relu) -> max_pool -> conv(with relu) -> max_pool ->
# -> conv(with relu) -> max_pool -> dense layer -> [output(train), softmax(main predictionss)]}
def build_model(input_val, w, b):
    ...
    out = tf.nn.bias_add(tf.matmul(dense1, w['w5']), b['b5'])   # used for training the CNN model
    softmax = tf.nn.softmax(out)                                # used after training the CNN

    return out, softmax


def main(num_epochs=85):        # 100 Seemed to be over trained
    """
    train and predict network outputs output on random image
    :param num_epochs:
    :return:
    """
    x = tf.placeholder(tf.float32, [None, 32, 32, 3])   # initialize input shape and labels
    y = tf.placeholder(tf.int32, [None, 10])

    weights = {     # initialize weights for each different layers
    ...
    biases = {      # initialize biases for each layers
    ...
    error = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predict,  # back propagation process
                                                                   labels=y))
    optm = tf.train.AdamOptimizer(learning_rate=0.01).minimize(error)
    corr = tf.equal(tf.argmax(predict, 1), tf.argmax(y, 1))
    accuracy = tf.reduce_mean(tf.cast(corr, tf.float32))
    saver = tf.train.Saver()        # initialize saver for saving weight and bias valuess

    pwd = os.getcwd()

    init = tf.global_variables_initializer()
    if not os.path.exists(pwd+'/Training/model.ckpt.meta'):
        sess = tf.Session()         # initialize tensorflow session
        sess.run(init)
        os.system('clear')
        # load dataset
        print '\033[1m\033[31m* Loading dataset... *\033[0m'
        X_train, y_train, X_val, y_val, X_test, y_test = load_dataset()

        print '\033[1m\033[32m* Starting Training *\033[0m'
        for epoch in range(num_epochs):
            ...
            for batch in iterate_minibatches(X_train, y_train, 500, shuffle=True):  # divide into mini batches
                inputs, targets = batch
                sess.run([optm], feed_dict={x: inputs, y: targets})                         # update weights
            ...
            print("Epoch {} of {} took {:.3f}s".format(     # print present epoch with total number of epoch
                epoch + 1, num_epochs, time.time() - start_time))
            print("  training loss:\t\t{:.6f}".format(train_err / train_batches))
            print("  validation loss:\t\t{:.6f}".format(val_err / val_batches))
            print("  validation accuracy:\t\t{:.2f} %".format(
                val_acc / val_batches * 100))               # print training and validation loss with accuracy
        ...
        save_path = saver.save(sess, "Training/model.ckpt")

    else:
        print '\033[1m\033[32m* Model is Pre-Trained *\033[0m'
        sess = tf.Session()
        sess.run(init)
        # restore weights value for this CNN
        saver.restore(sess, pwd + "/Training/model.ckpt")
    if len(sys.argv) > 1:
        sample_img = sys.argv[1]
    else:
        sample_img = 'plane.jpeg'
    ...
    # output prediction of image gives 10 numbers (class probabilities)
    prediction = sess.run(out_predict, feed_dict={x: img_})

    classify_name(prediction)   # print predicted class
    sess.close()
